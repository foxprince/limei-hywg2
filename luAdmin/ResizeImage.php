	...
	*/
	...
	public function setDestination($destination){
	    if(is_dir($destination) && is_writable($destination)) {
		   ...
		   if($last== '/' || $last=='\\'){
			...
			$this->_messages[]="Can not write to $destination.";
		}
	}
	...
	public function create(){
		if($this->_canProcess && $this->_originalwidth!=0){
			...
			$this->_message[]='Can not determine size of '.$this->_original;
		}
	}
	...
	protected function createThumbnail(){
		...
		if($this->_imageType=='jpeg'){
		    $newname.='.jpg';
			$success=imagejpeg($thumb, $this->_destination.$newname,92);
		}elseif($this->_imageType=='png'){
			$newname.='.png';
			$success=imagepng($thumb, $this->_destination.$newname,1);
		}elseif($this->_imageType=='gif'){
			$newname.='.gif';
