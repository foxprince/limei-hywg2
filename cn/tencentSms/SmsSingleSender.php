...
class SmsSingleSender
{
    ...
    public function __construct($appid, $appkey)
    {
        ...
        $this->iurl = "https://yun.tim.qq.com/v5/tlssmssvr/sendisms";
        $this->appid =  $appid;
        $this->appkey = $appkey;
     ...
     */
    public function sendi($type, $nationCode, $phoneNumber, $msg, $extend = "", $ext = "")
    {
        $random = $this->util->getRandom();
        $curTime = time();
        $wholeUrl = $this->iurl . "?sdkappid=" . $this->appid . "&random=" . $random;
        ...
        $data->type = (int)$type;
        $data->msg = $msg;
        //string sig = sha256(appkey=$appkey&random=random&time=$curTime&tel=$phoneNumber);
        $data->sig = hash("sha256",
            "appkey=".$this->appkey."&random=".$random."&time="
           .$curTime."&tel=".$phoneNumber, FALSE);
        $data->time = $curTime;
        $data->extend = $extend;
        $data->ext = $ext;
